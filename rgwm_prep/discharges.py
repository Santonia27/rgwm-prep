...
## Afvoer
def process_aanvoer_discharge(fn_path: str | Path, output_fn: str | Path, balance:bool):
    ...
    for file in glob.glob(f"{fn_path}/*"):
        if file.endswith(".csv") and "IN" in file:
            ...
            if balance:
                ...
                output = output_fn / "in" / f"VZM_{name}_IN_discharge.csv"
                with open(output, "w") as f:
...
def process_afvoer_discharge(fn_path: str | Path, output_fn: str | Path, balance: bool):
    ...
    for file in glob.glob(f"{fn_path}/*"):
        if file.endswith(".csv") and "OUT" in file:
            ...
            if balance:
                ...
                # Save .VZM input file
                output = output_fn / "out" / f"VZM_{name}_OUT_discharge.csv"
                with open(output, "w") as f:
                    f.write("# Waarnemingssoort,begindatum,begintijd,tijdstap in minuten\n")
                    f.write(f"# {name}\n")
                    f.write("H 2010-01-01 12:00 1440\n")
